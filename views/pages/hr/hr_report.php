		...
		<div class="col-lg-12 mt-3">
			<div class="card">
				...
				<div class="card-header d-flex align-items-center">
					<h1> Learning and Development (L&D) Reports</h1>
				</div>
				<div class="card-body">
								...
								<div class="card-body">
									<div class=row>
										<div class="col-lg-12">
											...
											<div class="table-responsive">          
												<table id ="tblReport4" class="table">
													...
													<tr>
														...
														<td>
														    <a href="#" class="open-names" data-names="<?= htmlspecialchars($row['names1'], ENT_QUOTES) ?>" data-toggle="modal" data-target="#namesModal" data-html="true" data-placement="top" title="<?= $row['names1'] ?>"> <?= $row['permanent'] ?></a>
														</td>
														<td>
															<a href="#" class="open-names" data-names="<?= htmlspecialchars($row['names'], ENT_QUOTES) ?>" data-toggle="modal" data-target="#namesModal" data-html="true" data-placement="top" title="<?= $row['names'] ?>"><?= $row['training_count']?></a>
														</td>
														<td><b>
											...
											<button type="button" class="btn btn-primary" onclick="htmlTableToExcel1('xlsx')"><i class="fa fa-file-excel-o"></i> Download Excel File</button>
										</div><!--Inner Col-lg-4-->
									</div>
								</div>
							</div>
								...
								<div class="card-body">
									<div class=row>
										<div class="col-lg-12">
											...
											<button type="button" class="btn btn-primary" onclick="htmlTableToExcel('xlsx')"><i class="fa fa-file-excel-o"></i> Download Excel File</button>
										</div><!--Inner Col-lg-4-->
									</div>
								</div>
							</div>
...
<!--Modal List of Names-->
    <div class="modal fade" id="namesModal" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
    
          <div class="modal-header">
            <h5 class="modal-title">List of Employees</h5>
    ...
    <script>
        $(document).on("click", ".open-names", function () {
            let names = $(this).data("names"); // string from PHP
...
<!------------Download Table report 5-------->
  ...
  <script>
    ...
    function htmlTableToExcel1(type, fn, dl) {
       var elt = document.getElementById('tblReport4');
       var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
       return dl ?
         XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }):
         XLSX.writeFile(wb, fn || ('Monitoring of Employees Training Attendance as of <?= date('m-d-Y')?>.' + (type || 'xlsx')));
    }
  </script>
<!------------Download-------->
