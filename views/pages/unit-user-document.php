...
<div class="content-inner">
		...
		<header class="page-header">
		  <div class="container-fluid">
			...
			<h2 class="no-margin-bottom"><?= $this->session->ous_desc; ?>: <?= $unit_desc; ?></h2>
		  </div>
		</header>
									...
									<button data-toggle="dropdown" type="button" class="btn btn-outline-primary dropdown-toggle btn-sm pull-right"><i class="fa fa-tasks"></i> Action <span class="caret"></span></button>
											<div class="dropdown-menu no-padding-bottom">
												<?php foreach($category as $row){ ?>
													<a href="<?= base_url();?>unit_user_document_add/<?= $back_link; ?>-<?= $i; ?>-<?= $row['cat_id']?>-<?= $row['cat_desc']?>" type="submit" class="dropdown-item"><i class="fa fa-calendar-plus-o"></i> <?= $row['cat_desc']?></a>
												<?php } ?>
											</div>
									</li>
